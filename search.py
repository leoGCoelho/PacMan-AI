...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not queue.isEmpty():  # Enquanto a fila nao for vazia
        ...
        if problem.isGoalState(node): # Se o nodo atual é o nodo objetivo, achou o objetivo e retorna os movimentos
            return mov

        if node in exp: # Se o nodo atual tive sido expandido, continua a procura no proximo
            continue

        # Parametros do proximo nodos
        exp.add(node)
        next = problem.getSuccessors()
        listN = list(next)
        ...
        for n in listN:
            ...
            queue.push((n[0], mov+n[1], costUpdated+n[2]), (costUpdated+heuristic(n[0], problem)+n[2]))
